...
import { GoogleAuthModal } from '@/components/pram/sign-in-gate'
    ...
    <>
      <header className="sticky top-0 z-30 flex h-16 w-full items-center justify-between border-b border-slate-200 bg-white/95 px-4 backdrop-blur-md sm:px-8">
        ...
        <nav className="hidden md:flex items-center gap-2 lg:gap-3">
          ...
          {/* Botón Iniciar Sesión (Abre Modal con Google) */}
          <button
            type="button"
            onClick={() => setAuthModalMode('sign-in')}
            className="inline-flex items-center gap-1.5 text-xs font-medium text-slate-700 hover:text-slate-900 bg-slate-100 hover:bg-slate-200/80 transition-colors px-3 py-2 rounded-lg border border-slate-200 cursor-pointer ml-1"
          >
            <LogIn className="size-3.5 text-slate-600" />
            <span>Iniciar Sesión</span>
          </button>

          {/* Botón Registrarse (Abre Modal con Google) */}
          <button
            type="button"
            onClick={() => setAuthModalMode('sign-up')}
            className="inline-flex items-center gap-1.5 text-xs font-semibold text-white bg-[#152642] hover:bg-[#1e3a5f] transition-all px-3.5 py-2 rounded-lg shadow-sm cursor-pointer"
          >
